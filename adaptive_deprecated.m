function [x_new, dt, dt_new, adap_step_count, bjoerck_count, crossed] = adaptive_deprecated(x_old, t_old, P, dt, L, R, B, Cd, Cv, crossed, METHOD_num, METHOD_diff)
    %{
    INPUTS:
    ...
    % Perform adaptive time step only if the previous (ten to be sure)
    % steps had differentiable RHS. Otherwise keep the step size fixed.
    if not(crossed)
        while (not(complete_step))
            ...
            if (DEN > tol_adaptive)
                % do while loop again one more time with half the time step
                dt = dt/2; 
            else                % the step is differentiable. 2 cases
                dt_new = dt;
                complete_step = 1;
                if DEN < tol_adaptive/3
                    dt_new = 1.1*dt;   
                    %next = nthroot(((tol)/(2*DEN)), 4)
                    %dt_new = dt*next; 
                end
            end
        end
        ...
        crossed = crossed - 1;
        [x_new, ~] = METHOD_num(METHOD_diff, x_old, P, dt, L, R, B, Cd, Cv);
        dt_new = 1e-4;
    end
