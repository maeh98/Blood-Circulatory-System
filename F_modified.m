function y = F_modified(x, P, flag, L, R, B, Cd, Cv)   
    %{
    INPUTS:
    ...
    time.
    NOTE:
    Compared to function F, this function resolves the non
    differentiability by modifiying the turbolent term. 
    |Q|*Q is approximated with sqrt(Q^4 + 10^-7)
    %}
    ...
    % Do fluxes Qe
    y(15:28) = ((1./L(1:14)) .* (-[P(2:14); P(1)] + P(1:14) - R(1:14).*Q(1:14))); 
    % inside heart nonlinearity
    y(20:21) = y(20:21) - ((1./L(6:7)) .* (B(1:2).*Q(6:7).*sqrt(Q(6:7).^2 +1e-8)));
    y(27:28) = y(27:28) - ((1./L(13:14)) .* (B(3:4).*Q(13:14).*sqrt(Q(13:14).^2 + 1e-8)));
